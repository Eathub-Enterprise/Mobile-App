...
export default class Endpoints {
    constructor() {
        // this.url=process.env.PORT.trim()
    ...
    async get_or_update_or_remove_FavouriteMeals(st, addMessage, method, refresh_stFunc,foodItemid,setAnimation) {
        if (["Post", "Put"].includes(method)) {
            ...
            try {
                console.log("faf")
                let response = await fetch(`${this.url}api/v1/favourites/null/`, this.requestOptions)
    ...
    async TrackOrder(st, method, addMessage, currentPagestate, currentPage_stFunc, foodCurrentstate, refresh_stFunc) {
